...
public class JFrameAdminHariRabu extends javax.swing.JFrame {
    ...
    public JFrameAdminHariRabu() {
        ...
        jTableAdminHariRabu.getTableHeader().setFont(new Font("SEGOE UI", Font.BOLD, 12));
    ...
    public void ComboBoxCariKelas() {
        try {
            String sql = "SELECT DISTINCT kelas FROM tb_jadwal_mapel WHERE " + " hari LIKE '%" + "Rabu" + "%' ORDER BY kelas ASC ";
    ...
    private void LoadTable() {
        ...
        try {
            //int no = 1;
            String sql = "SELECT * FROM tb_jadwal_mapel WHERE " + " hari LIKE '%" + "Rabu" + "%' ORDER BY nama_guru ASC ";
            java.sql.Connection conn = (Connection) Config.configDB();
            java.sql.Statement stm = conn.createStatement();
            java.sql.ResultSet res = stm.executeQuery(sql);
            while (res.next()) {
            ...
            jTableAdminHariRabu.setModel(model);
        } catch (Exception e) {
        }
    }
    ...
    private void initComponents() {
        ...
        jLabel1.setText("Jadwal Pelajaran Hari Rabu");
        ...
        jTableAdminHariRabu.setShowVerticalLines(false);
        jTableAdminHariRabu.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                jTableAdminHariRabuMouseClicked(evt);
            }
        });
    ...
    private void Btn_CariActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_Btn_CariActionPerformed
        try {
            ...
            ResultSet res = stat.executeQuery("SELECT * FROM tb_jadwal_mapel WHERE " + " nama_guru LIKE '%" + Txt_Cari.getText() + "%' AND " + " hari LIKE '%" + "Rabu" + "%' OR " + " kelas LIKE '%" + Txt_Cari.getText() + "%' AND " + " hari LIKE '%" + "Rabu" + "%' ORDER BY nama_guru ASC ");
            DefaultTableModel model = new DefaultTableModel();
            model.addColumn("ID");
            model.addColumn("Guru Pengajar");
            model.addColumn("Sesi");
    ...
    private void Btn_PrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_Btn_PrintActionPerformed
        try {
            jTableAdminHariRabu.print(JTable.PrintMode.FIT_WIDTH, new MessageFormat("JADWAL PELAJARAN HARI RABU"), null);
        } catch (PrinterException ex) {
            Logger.getLogger(JFrameAdminHariRabu.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    private void jTableAdminHariRabuMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jTableAdminHariRabuMouseClicked
        int baris = jTableAdminHariRabu.rowAtPoint(evt.getPoint());
        selectedId = jTableAdminHariRabu.getValueAt(baris, 0).toString();
    }//GEN-LAST:event_jTableAdminHariRabuMouseClicked

    private void Btn_Cari_KelasActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_Btn_Cari_KelasActionPerformed
        try {
            Statement stat = (Statement) Config.configDB().createStatement();
            ResultSet res = stat.executeQuery("SELECT * FROM tb_jadwal_mapel WHERE " + " kelas LIKE '%" + Cmbx_Cari_Kelas.getSelectedItem()+ "%' AND " + " hari LIKE '%" + "Rabu" + "%' ORDER BY nama_guru ASC ");
            DefaultTableModel model = new DefaultTableModel();
            model.addColumn("ID");
            model.addColumn("Guru Pengajar");
            model.addColumn("Sesi");
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(JFrameAdminHariRabu.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
