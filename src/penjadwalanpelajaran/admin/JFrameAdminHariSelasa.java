...
public class JFrameAdminHariSelasa extends javax.swing.JFrame {
    ...
    public void ComboBoxCariKelas() {
        try {
            String sql = "SELECT DISTINCT kelas FROM tb_jadwal_mapel WHERE " + " hari LIKE '%" + "Selasa" + "%' ORDER BY kelas ASC ";
    ...
    private void Btn_PrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_Btn_PrintActionPerformed
        try {
            jTableAdminHariSelasa.print(JTable.PrintMode.FIT_WIDTH, new MessageFormat("JADWAL PELAJARAN HARI SELASA"), null);
        } catch (PrinterException ex) {
            Logger.getLogger(JFrameAdminHariSelasa.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    private void Btn_Cari_KelasActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_Btn_Cari_KelasActionPerformed
        try {
            Statement stat = (Statement) Config.configDB().createStatement();
            ResultSet res = stat.executeQuery("SELECT * FROM tb_jadwal_mapel WHERE " + " kelas LIKE '%" + Cmbx_Cari_Kelas.getSelectedItem()+ "%' AND " + " hari LIKE '%" + "selasa" + "%' ORDER BY nama_guru ASC ");
            DefaultTableModel model = new DefaultTableModel();
            model.addColumn("ID");
            model.addColumn("Guru Pengajar");
            model.addColumn("Sesi");
